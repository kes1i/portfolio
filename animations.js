...
function initHero3DScene() {
    ...
    // Create a creeper head group
    const creeperGroup = new THREE.Group();
    scene.add(creeperGroup);
    
    // Function to create texture from Creeper 
    function createCreeperTexture() {
        ...
        // Mouth
        ctx.fillRect(192, 256, 64, 64);
        ctx.fillRect(256, 256, 64, 64);
        ctx.fillRect(192, 320, 64, 64);
        ctx.fillRect(256, 320, 64, 64);
        
        // Create pixel pattern
        ctx.fillStyle = '#4A7B33';
        for (let i = 0; i < 20; i++) {
            const x = Math.floor(Math.random() * 16) * 32;
    ...
    // Create the creeper texture
    const textureCanvas = createCreeperTexture();
    const creeperTexture = new THREE.CanvasTexture(textureCanvas);
    creeperTexture.magFilter = THREE.NearestFilter; // Keep the pixel art look
    creeperTexture.minFilter = THREE.NearestFilter;
    
    // Create materials for different sides of the creeper head
    const headMaterials = [
        new THREE.MeshBasicMaterial({ map: creeperTexture }), // right
        new THREE.MeshBasicMaterial({ map: creeperTexture }), // left
        new THREE.MeshBasicMaterial({ map: creeperTexture }), // top
        new THREE.MeshBasicMaterial({ map: creeperTexture }), // bottom
        new THREE.MeshBasicMaterial({ map: creeperTexture }), // front - face
        new THREE.MeshBasicMaterial({ map: creeperTexture })  // back
    ];
    
    // Create creeper head only
    const headGeometry = new THREE.BoxGeometry(4, 4, 4);
    const creeperHead = new THREE.Mesh(headGeometry, headMaterials);
    creeperGroup.add(creeperHead);
    
    // Position the creeper head
    creeperGroup.position.y = 0; // Adjust for head-only view
    creeperGroup.position.z = 0;
    ...
    function animate() {
        ...
        // Rotate the creeper head
        creeperGroup.rotation.y = Math.sin(time * 0.5) * 0.5;
        
        // Floating animation
        creeperGroup.position.y = Math.sin(time * 0.8) * 0.5;
